...
void UseShadow(int shader, Matrix projection, Matrix viewmat, Matrix modelmat, Matrix modelviewinv, float mvLightPos[3], float lightDir[3])
{
	...
	//glUniformMatrix4fvARB(s->m_slot[SSLOT_NORMALMAT], 1, 0, modelviewinv.m_matrix);
	//glUniformMatrix4fvARB(s->m_slot[SSLOT_INVMODLVIEWMAT], 1, 0, modelviewinv.m_matrix);
	...
	//glUniformMatrix4fvARB(s->m_slot[SSLOT_LIGHTMATRIX], 1, false, g_lightmat);
	glUniformMatrix4fvARB(s->m_slot[SSLOT_LIGHTMATRIX], 1, false, g_lightmat.m_matrix);
...
void RenderShadowedScene(Matrix projection, Matrix viewmat, Matrix modelmat, Matrix modelview)
{
	...
	{
		UseS(COLOR3D);
		glUniformMatrix4fvARB(g_shader[SHADER_COLOR3D].m_slot[SSLOT_PROJECTION], 1, 0, projection.m_matrix);
		glUniformMatrix4fvARB(g_shader[SHADER_COLOR3D].m_slot[SSLOT_MODELMAT], 1, 0, modelmat.m_matrix);
		glUniformMatrix4fvARB(g_shader[SHADER_COLOR3D].m_slot[SSLOT_VIEWMAT], 1, 0, viewmat.m_matrix);
