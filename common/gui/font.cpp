...
void DrawGlyph()
{
	...
	if(g_rpartit->m_type == RICHTEXT_ICON)
	{
		...
		//g_applog<<"color[3] = "<<currcolor[3]<<std::endl;
	}
...
void AdvGlyph()
{
	...
	//if(debugtest)	g_applog<<"x adv "<<x<<std::endl;
}
...
void LoadFont(int id, const char* fontfile)
{
	...
	if(!fp)
	{
		...
		g_applog<<"Full path: "<<fullfontpath<<std::endl;
		return;
	}
...
int EndX(const RichText* text, int lastc, int fnt, float startx, float starty)
{
	...
	//g_applog<<"g_localP= "<<g_localP<<" g_currw*100 = "<<g_currw*100<<"   g_width="<<g_width<<" "<<std::endl;
	...
	for(i=0; i<size && i<lastc; i++)
	{
		...
		//g_applog<<"g_str[i] = "<<g_str[i]<<std::endl;
		AdvGlyph();
...
void LoadFonts()
{
	...
	LoadFont(FONT_MSUIGOTHIC16, "fonts/msuigothic16");
	//LoadFont(FONT_MSUIGOTHIC16, "fonts/gulim16");
}
