...
Button::Button(Widget* parent, const char* name, const char* filepath, const std::string label, const std::string tooltip, int f, int style, void (*reframef)(Widget* thisw), void (*click)(), void (*click2)(int p), void (*overf)(), void (*overf2)(int p), void (*out)(), int parm) : Widget()
{
	m_parent = parent;
	m_type = WIDGET_BUTTON;
	m_name = name;
	...
	length = EndX(m_text.c_str(), m_text.length(), m_font, 0, 0);
	//m_tpos[0] = (left+right)/2.0f - length/2.0f;
	//m_tpos[1] = (top+bottom)/2.0f - g_font[f].gheight/2.0f;
	m_over = false;
	m_ldown = false;
...
void Button::draw()
{
	...
	else if(m_style == BUTTON_LEFTIMAGE)
	{
		...
		DrawShadowedText(m_font, textleft, texttop, m_label.c_str());
	}
