...
void FilterTest::draw_fresponse() {
  ...
  the_filter->init(filt->cutoff,filt->fq,filt_qfactor->value(),&filt_qfactor->text);
...
FilterTest::FilterTest():
  ...
  test_filter=new RButtons(filt_ctr,0,Rect(10,18,120,5*TDIST),"filter",true,test_filt_cmd);
  ...
  filt_freq=new HSlider(filt_ctr,1,Rect(92,130,138,0),12,"corner freq",
    [](HSlider* sl,int fire,bool rel) {
